...
// Since node is required for nxpkg development, it makes a decent
// candidate.
func testBin() string {
...
func waitForFile(t *testing.T, filename nxpkgpath.AbsoluteSystemPath, timeout time.Duration) {
	t.Helper()
...
func TestDaemonLifecycle(t *testing.T) {
	...
	wg.Add(1)
	go func() {
		serverErr = d.runNxpkgServer(ctx, ts, watcher)
		wg.Done()
	}()
	...
	cancel()
	wg.Wait()
	assert.NilError(t, serverErr, "runNxpkgServer")
...
func TestTimeout(t *testing.T) {
	...
	err := d.runNxpkgServer(ctx, ts, watcher)
...
func TestCaughtSignal(t *testing.T) {
	...
	// registered with the nxpkgserver, we've registered all of our
	...
	// are acceptable outcomes for runNxpkgServer. Any other error, or a timeout, is a
	// failure.
	watcher.Close()
	...
	if err != nil && !errors.Is(err, grpc.ErrServerStopped) {
		t.Errorf("runNxpkgServer got err %v, want nil or ErrServerStopped", err)
	}
}
