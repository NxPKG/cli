...
import (
	...
	"github.com/nxpkg/cli/internal/daemon/connector"
	"github.com/nxpkg/cli/internal/fs"
	"github.com/nxpkg/cli/internal/server"
...
func getRepoHash(repoRoot nxpkgpath.AbsoluteSystemPath) string {
...
func getDaemonFileRoot(repoRoot nxpkgpath.AbsoluteSystemPath) nxpkgpath.AbsoluteSystemPath {
	tempDir := fs.TempDir("nxpkgd")
	hexHash := getRepoHash(repoRoot)
	return tempDir.UntypedJoin(hexHash)
}

func getLogFilePath(repoRoot nxpkgpath.AbsoluteSystemPath) (nxpkgpath.AbsoluteSystemPath, error) {
	hexHash := getRepoHash(repoRoot)
	base := repoRoot.Base()
	logFilename := fmt.Sprintf("%v-%v.log", hexHash, base)

	logsDir := fs.GetNxpkgDataDir().UntypedJoin("logs")
	return logsDir.UntypedJoin(logFilename), nil
}

func getUnixSocket(repoRoot nxpkgpath.AbsoluteSystemPath) nxpkgpath.AbsoluteSystemPath {
	root := getDaemonFileRoot(repoRoot)
	return root.UntypedJoin("nxpkgd.sock")
}

func getPidFile(repoRoot nxpkgpath.AbsoluteSystemPath) nxpkgpath.AbsoluteSystemPath {
	root := getDaemonFileRoot(repoRoot)
	return root.UntypedJoin("nxpkgd.pid")
}
...
func ExecuteDaemon(ctx context.Context, helper *cmdutil.Helper, signalWatcher *signals.Watcher, executionState *nxpkgstate.ExecutionState) error {
	...
	nxpkgServer, err := server.New(serverName, d.logger.Named("rpc server"), base.RepoRoot, base.NxpkgVersion, logFilePath)
	if err != nil {
		d.logError(err)
		return err
	}
	defer func() { _ = nxpkgServer.Close() }()
	err = d.runNxpkgServer(ctx, nxpkgServer, signalWatcher)
	if err != nil {
		d.logError(err)
		return err
	}
...
var errInactivityTimeout = errors.New("nxpkgd shut down from inactivity")
...
func tryAcquirePidfileLock(pidPath nxpkgpath.AbsoluteSystemPath) (lockfile.Lockfile, error) {
...
func (d *daemon) runNxpkgServer(parentContext context.Context, rpcServer rpcServer, signalWatcher *signals.Watcher) error {
	ctx, cancel := context.WithCancel(parentContext)
	defer cancel()
	pidPath := getPidFile(d.repoRoot)
	lock, err := tryAcquirePidfileLock(pidPath)
	if err != nil {
		return errors.Wrapf(err, "failed to lock the pid file at %v. Is another nxpkg daemon running?", lock)
	}
...
func GetClient(ctx context.Context, repoRoot nxpkgpath.AbsoluteSystemPath, logger hclog.Logger, nxpkgVersion string, opts ClientOpts) (*Client, error) {
	...
	if strings.HasSuffix(bin, "go-nxpkg") {
		bin = filepath.Join(filepath.Dir(bin), "nxpkg")
	} else if strings.HasSuffix(bin, "go-nxpkg.exe") {
		bin = filepath.Join(filepath.Dir(bin), "nxpkg.exe")
	}
	c := &connector.Connector{
		Logger:       logger.Named("NxpkgdClient"),
		Bin:          bin,
		Opts:         opts,
		SockPath:     sockPath,
		PidPath:      pidPath,
