...
// PackageType represents the mode in which nxpkg is running.
...
func candidateDirectoryWorkspaceGlobs(directory nxpkgpath.AbsoluteSystemPath) []string {
...
func isOneOfTheWorkspaces(globs []string, nearestPackageJSONDir nxpkgpath.AbsoluteSystemPath, currentPackageJSONDir nxpkgpath.AbsoluteSystemPath) bool {
...
// nxpkg should be in when operating at that directory.
func InferRoot(directory nxpkgpath.AbsoluteSystemPath) (nxpkgpath.AbsoluteSystemPath, PackageType) {
	// Go doesn't have iterators, so this is very not-elegant.

	// Scenarios:
	// 0. Has a nxpkg.json but doesn't have a peer package.json. directory + multi
	// 1. Nearest nxpkg.json, check peer package.json/pnpm-workspace.yaml.
	//    A. Has workspaces, multi package mode.
	//    B. No workspaces, single package mode.
	// 2. If no nxpkg.json find the closest package.json parent.
	...
	nearestNxpkgJSON, findNxpkgJSONErr := directory.Findup("nxpkg.json")
	if nearestNxpkgJSON == "" || findNxpkgJSONErr != nil {
		// We didn't find a nxpkg.json. We're in situation 2 or 3.
		...
		siblingPackageJSONPath := nearestNxpkgJSON.Dir().UntypedJoin("package.json")
		...
		if candidateDirectoryWorkspaceGlobs(nearestNxpkgJSON.Dir()) != nil {
			// Scenario 1A.
			return nearestNxpkgJSON.Dir(), Multi
		}

		// Scenario 1B.
		return nearestNxpkgJSON.Dir(), Single
	}
}
