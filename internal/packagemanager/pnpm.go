...
import (
	...
	"github.com/nxpkg/cli/internal/yaml"
)
...
func readPnpmWorkspacePackages(workspaceFile nxpkgpath.AbsoluteSystemPath) ([]string, error) {
...
func getPnpmWorkspaceGlobs(rootpath nxpkgpath.AbsoluteSystemPath) ([]string, error) {
	...
	if len(pkgGlobs) == 0 {
		return nil, fmt.Errorf("pnpm-workspace.yaml: no packages found. Nxpkgrepo requires pnpm workspaces and thus packages to be defined in the root pnpm-workspace.yaml")
	}
...
func getPnpmWorkspaceIgnores(pm PackageManager, rootpath nxpkgpath.AbsoluteSystemPath) ([]string, error) {
...
func pnpmPrunePatches(pkgJSON *fs.PackageJSON, patches []nxpkgpath.AnchoredUnixPath) error {
	pkgJSON.Mu.Lock()
