...
import (
	...
	"github.com/nxpkg/cli/internal/packagemanager"
...
func (m *mockLockfile) ResolvePackage(workspacePath nxpkgpath.AnchoredUnixPath, name string, version string) (lockfile.Package, error) {
...
func (m *mockLockfile) Subgraph(workspaces []nxpkgpath.AnchoredSystemPath, packages []string) (lockfile.Lockfile, error) {
	return nil, nil
}
...
func TestResolvePackages(t *testing.T) {
	...
	workspaceInfos := workspace.Catalog{
		PackageJSONs: map[string]*fs.PackageJSON{
			...
			"app1": {
				Dir:  nxpkgpath.AnchoredUnixPath("app/app1").ToSystemPath(),
				Name: "app1",
			},
			"app2": {
				Dir:  nxpkgpath.AnchoredUnixPath("app/app2").ToSystemPath(),
				Name: "app2",
			},
			"app2-a": {
				Dir:  nxpkgpath.AnchoredUnixPath("app/app2-a").ToSystemPath(),
				Name: "app2-a",
			},
			"libA": {
				Dir:  nxpkgpath.AnchoredUnixPath("libs/libA").ToSystemPath(),
				Name: "libA",
			},
			"libB": {
				Dir:                    nxpkgpath.AnchoredUnixPath("libs/libB").ToSystemPath(),
				Name:                   "libB",
			...
			"libC": {
				Dir:  nxpkgpath.AnchoredUnixPath("libs/libC").ToSystemPath(),
				Name: "libC",
			},
			"libD": {
				Dir:  nxpkgpath.AnchoredUnixPath("libs/libD").ToSystemPath(),
				Name: "libD",
			},
		},
	}
	...
	testCases := []struct {
		...
		{
			name:                "Only nxpkg.json changed",
			changed:             []string{"nxpkg.json"},
		...
		},
		{
			...
			// prevents this issue: https://github.com/vercel/nxpkg/issues/1528
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("test #%v %v", i, tc.name), func(t *testing.T) {
			...
			pkgs, isAllPackages, err := ResolvePackages(&Opts{
				...
				PackageManager: &packagemanager.PackageManager{Lockfile: tc.lockfile, UnmarshalLockfile: readLockfile, GetLockfileName: func(_ nxpkgpath.AbsoluteSystemPath) string { return tc.lockfile }},
