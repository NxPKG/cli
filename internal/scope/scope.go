...
import (
	...
	"github.com/nxpkg/cli/internal/context"
	"github.com/nxpkg/cli/internal/lockfile"
	"github.com/nxpkg/cli/internal/scm"
	scope_filter "github.com/nxpkg/cli/internal/scope/filter"
	"github.com/nxpkg/cli/internal/nxpkgpath"
	"github.com/nxpkg/cli/internal/nxpkgstate"
	"github.com/nxpkg/cli/internal/util"
	"github.com/nxpkg/cli/internal/util/filter"
	"github.com/nxpkg/cli/internal/workspace"
)
...
func addLegacyFlagsFromArgs(opts *LegacyFilter, args *nxpkgstate.ParsedArgsFromRust) {
...
// Opts holds the options for how to select the entrypoint packages for a nxpkg run
type Opts struct {
	...
	PackageInferenceRoot nxpkgpath.RelativeSystemPath
}
...
nxpkg's documentation https://nxpkg.github.io/repo/docs/reference/command-line-reference/run#--filter
...
in the root directory. Includes nxpkg.json, root package.json, and the root lockfile by default.`
)
...
func resolvePackageInferencePath(raw string) (nxpkgpath.RelativeSystemPath, error) {
	pkgInferenceRoot, err := nxpkgpath.CheckedToRelativeSystemPath(raw)
	if err != nil {
...
func OptsFromArgs(opts *Opts, args *nxpkgstate.ParsedArgsFromRust) error {
...
func ResolvePackages(opts *Opts, repoRoot nxpkgpath.AbsoluteSystemPath, scm scm.SCM, ctx *context.Context, tui cli.Ui, logger hclog.Logger) (util.Set, bool, error) {
	...
	filterResolver := &scope_filter.Resolver{
		...
		PackagesChangedInRange: opts.getPackageChangeFunc(scm, repoRoot, ctx, logger),
		Logger:                 logger,
	}
	...
	logger.Debug("filter patterns", "patterns", filterPatterns)
	...
	if isAllPackages {
		logger.Debug("No filters specified, running all packages")
	...
	logger.Debug("filtered packages", "packages", filteredPkgs)
	return filteredPkgs, isAllPackages, nil
}

func calculateInference(repoRoot nxpkgpath.AbsoluteSystemPath, pkgInferencePath nxpkgpath.RelativeSystemPath, packageInfos workspace.Catalog, logger hclog.Logger) (*scope_filter.PackageInference, error) {
	if pkgInferencePath == "" {
...
func (o *Opts) getPackageChangeFunc(scm scm.SCM, repoRoot nxpkgpath.AbsoluteSystemPath, ctx *context.Context, logger hclog.Logger) scope_filter.PackagesChangedInRange {
	return func(fromRef string, toRef string) (util.Set, error) {
		...
		if hasRepoGlobalFileChanged, err := repoGlobalFileHasChanged(o, getDefaultGlobalDeps(), changedFiles); err != nil {
			...
			logger.Debug("Global dependencies have changed, running all packages")
		...
		logger.Debug("Changed packages", "packages", changedPkgs)
		...
		if lockfileChanges, fullChanges := getChangesFromLockfile(repoRoot, scm, ctx, changedFiles, fromRef); !fullChanges {
			for _, pkg := range lockfileChanges {
				logger.Debug("adding package from lockfile", "package", pkg)
				changedPkgs.Add(pkg)
			}
		} else {
			return makeAllPkgs(), nil
...
func getChangesFromLockfile(repoRoot nxpkgpath.AbsoluteSystemPath, scm scm.SCM, ctx *context.Context, changedFiles []string, fromRef string) ([]string, bool) {
...
func getDefaultGlobalDeps() []string {
	// include nxpkg.json and root package.json as implicit global dependencies
