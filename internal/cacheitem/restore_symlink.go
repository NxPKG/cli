...
func restoreSymlink(dirCache *cachedDirTree, anchor nxpkgpath.AbsoluteSystemPath, header *tar.Header) (nxpkgpath.AnchoredSystemPath, error) {
...
func restoreSymlinkMissingTarget(dirCache *cachedDirTree, anchor nxpkgpath.AbsoluteSystemPath, header *tar.Header) (nxpkgpath.AnchoredSystemPath, error) {
	processedName, canonicalizeNameErr := canonicalizeName(header.Name)
	if canonicalizeNameErr != nil {
		return "", canonicalizeNameErr
	}
...
func actuallyRestoreSymlink(dirCache *cachedDirTree, anchor nxpkgpath.AbsoluteSystemPath, processedName nxpkgpath.AnchoredSystemPath, header *tar.Header) (nxpkgpath.AnchoredSystemPath, error) {
...
func topologicallyRestoreSymlinks(dirCache *cachedDirTree, anchor nxpkgpath.AbsoluteSystemPath, symlinks []*tar.Header, tr *tar.Reader) ([]nxpkgpath.AnchoredSystemPath, error) {
	restored := make([]nxpkgpath.AnchoredSystemPath, 0)
...
func canonicalizeLinkname(anchor nxpkgpath.AbsoluteSystemPath, processedName nxpkgpath.AnchoredSystemPath, linkname string) string {
	...
	// Take for example: `/Users/nxpkgbot/weird-filenames/\foo\/lol`
