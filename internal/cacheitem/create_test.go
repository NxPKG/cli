...
func createEntry(t *testing.T, anchor nxpkgpath.AbsoluteSystemPath, fileDefinition createFileDefinition) error {
	t.Helper()
...
func createDir(t *testing.T, anchor nxpkgpath.AbsoluteSystemPath, fileDefinition createFileDefinition) error {
	t.Helper()
...
func createFile(t *testing.T, anchor nxpkgpath.AbsoluteSystemPath, fileDefinition createFileDefinition) error {
	t.Helper()
	path := fileDefinition.Path.RestoreAnchor(anchor)
...
func createSymlink(t *testing.T, anchor nxpkgpath.AbsoluteSystemPath, fileDefinition createFileDefinition) error {
	t.Helper()
	path := fileDefinition.Path.RestoreAnchor(anchor)
...
func TestCreate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			files: []createFileDefinition{
				{
					Path:     nxpkgpath.AnchoredSystemPath("hello world.txt"),
					FileMode: 0 | 0644,
				},
			},
		...
		},
		{
			...
			files: []createFileDefinition{
				{
					Path:     nxpkgpath.AnchoredSystemPath("one"),
					Linkname: "two",
					FileMode: 0 | os.ModeSymlink | 0777,
				},
				{
					Path:     nxpkgpath.AnchoredSystemPath("two"),
					Linkname: "three",
					FileMode: 0 | os.ModeSymlink | 0777,
				},
				{
					Path:     nxpkgpath.AnchoredSystemPath("three"),
					Linkname: "real",
					FileMode: 0 | os.ModeSymlink | 0777,
				},
				{
					Path:     nxpkgpath.AnchoredSystemPath("real"),
					FileMode: 0 | 0644,
				},
			},
		...
		},
		{
			...
			files: []createFileDefinition{
				{
					Path:     nxpkgpath.AnchoredSystemPath("parent"),
					FileMode: 0 | os.ModeDir | 0755,
				},
				{
					Path:     nxpkgpath.AnchoredSystemPath("parent/child"),
					FileMode: 0 | 0644,
				},
			},
		...
		},
		{
			...
			files: []createFileDefinition{
				{
					Path:     nxpkgpath.AnchoredSystemPath("fifo"),
	...
	for _, tt := range tests {
		getTestFunc := func(compressed bool) func(t *testing.T) {
			return func(t *testing.T) {
				inputDir := nxpkgpath.AbsoluteSystemPath(t.TempDir())
				archiveDir := nxpkgpath.AbsoluteSystemPath(t.TempDir())
				var archivePath nxpkgpath.AbsoluteSystemPath
				if compressed {
					archivePath = nxpkgpath.AnchoredSystemPath("out.tar.zst").RestoreAnchor(archiveDir)
				} else {
					archivePath = nxpkgpath.AnchoredSystemPath("out.tar").RestoreAnchor(archiveDir)
				}
