...
type restoreFile struct {
	Name     nxpkgpath.AnchoredUnixPath
	Linkname string
	fs.FileMode
}

// generateTar is used specifically to generate tar files that Nxpkgrepo would
...
func generateTar(t *testing.T, files []tarFile) nxpkgpath.AbsoluteSystemPath {
	t.Helper()
	testDir := nxpkgpath.AbsoluteSystemPath(t.TempDir())
...
func compressTar(t *testing.T, archivePath nxpkgpath.AbsoluteSystemPath) nxpkgpath.AbsoluteSystemPath {
	t.Helper()
...
func generateAnchor(t *testing.T) nxpkgpath.AbsoluteSystemPath {
	t.Helper()
	testDir := nxpkgpath.AbsoluteSystemPath(t.TempDir())
...
func assertFileExists(t *testing.T, anchor nxpkgpath.AbsoluteSystemPath, diskFile restoreFile) {
	t.Helper()
...
func TestOpen(t *testing.T) {
	...
	type wantOutput struct {
		unix    []nxpkgpath.AnchoredSystemPath
		windows []nxpkgpath.AnchoredSystemPath
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			wantOutput: wantOutput{
				unix: nxpkgpath.AnchoredUnixPathArray{
					"one",
					"one/two",
					"one/two/three",
		...
		},
		{
			...
			wantOutput: wantOutput{
				unix: nxpkgpath.AnchoredUnixPathArray{"target", "source"}.ToSystemPathArray(),
			},
		},
		{
			name: "nested file",
			...
			wantOutput: wantOutput{
				unix: nxpkgpath.AnchoredUnixPathArray{"folder", "folder/file"}.ToSystemPathArray(),
			},
		},
		{
			name: "nested symlink",
			...
			wantOutput: wantOutput{
				unix: nxpkgpath.AnchoredUnixPathArray{"folder", "folder/symlink", "folder/symlink/folder-sibling"}.ToSystemPathArray(),
			},
		},
		{
			...
			wantOutput: wantOutput{
				unix: nxpkgpath.AnchoredUnixPathArray{"real", "three", "two", "one"}.ToSystemPathArray(),
			},
		},
		{
			...
			wantOutput: wantOutput{
				unix: nxpkgpath.AnchoredUnixPathArray{"folder-not-file", "folder-not-file/subfile"}.ToSystemPathArray(),
			},
			wantErr: wantErr{
				unix:    syscall.EISDIR,
		...
		// 	},
		// 	wantOutput: wantOutput{
		// 		unix:    nxpkgpath.AnchoredUnixPathArray{"one"}.ToSystemPathArray(),
		// 		windows: nil,
		// 	},
		// 	wantErr: wantErr{
		// 		unix:    os.ErrExist,
		...
		},
		{
			...
			wantOutput: wantOutput{
				unix: nxpkgpath.AnchoredUnixPathArray{"real", "one"}.ToSystemPathArray(),
			},
		},
		{
			...
			wantOutput: wantOutput{
				unix: nxpkgpath.AnchoredUnixPathArray{"escape"}.ToSystemPathArray(),
			},
			wantErr: wantErr{
				unix:    errTraversal,
		...
		},
		{
			...
			wantOutput: wantOutput{
				unix:    nxpkgpath.AnchoredUnixPathArray{"back\\slash\\file"}.ToSystemPathArray(),
				windows: nxpkgpath.AnchoredUnixPathArray{}.ToSystemPathArray(),
			},
			wantErr: wantErr{
				unix:    nil,
	...
	for _, tt := range tests {
		getTestFunc := func(compressed bool) func(t *testing.T) {
			return func(t *testing.T) {
				var archivePath nxpkgpath.AbsoluteSystemPath
				if compressed {
...
func Test_canonicalizeLinkname(t *testing.T) {
	...
	anchor := nxpkgpath.AbsoluteSystemPath(filepath.Join("path", "to", "anchor"))

	tests := []struct {
		name             string
		processedName    nxpkgpath.AnchoredSystemPath
		linkname         string
		...
		{
			name:             "hello world",
			processedName:    nxpkgpath.AnchoredSystemPath("source"),
			linkname:         "target",
		...
		},
		{
			...
			processedName:    nxpkgpath.AnchoredUnixPath("child/source").ToSystemPath(),
...
func Test_canonicalizeName(t *testing.T) {
	tests := []struct {
		name     string
		fileName string
		want     nxpkgpath.AnchoredSystemPath
		wantErr  error
	}{
		{
			name:     "hello world",
...
func TestCacheItem_Restore(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: nxpkgpath.AnchoredUnixPathArray{"target", "source", "one", "one/two"}.ToSystemPathArray(),
		},
	}
	for _, tt := range tests {
		getTestFunc := func(compressed bool) func(t *testing.T) {
