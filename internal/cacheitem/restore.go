...
func Open(path nxpkgpath.AbsoluteSystemPath) (*CacheItem, error) {
...
func (ci *CacheItem) Restore(anchor nxpkgpath.AbsoluteSystemPath) ([]nxpkgpath.AnchoredSystemPath, error) {
	var tr *tar.Reader
	...
	dirCache := &cachedDirTree{
		anchorAtDepth: []nxpkgpath.AbsoluteSystemPath{anchor},
	}
...
func restoreEntry(dirCache *cachedDirTree, anchor nxpkgpath.AbsoluteSystemPath, header *tar.Header, reader *tar.Reader) (nxpkgpath.AnchoredSystemPath, error) {
...
func canonicalizeName(name string) (nxpkgpath.AnchoredSystemPath, error) {
	// Assuming this was a `nxpkg`-created input, we currently have an AnchoredUnixPath.
	...
	return nxpkgpath.AnchoredUnixPathFromUpstream(noTrailingSlash).ToSystemPath(), nil
}
