...
// DaemonClient provides access to higher-level functionality from the daemon to a nxpkg run.
...
type Status struct {
	...
	LogFile  nxpkgpath.AbsoluteSystemPath `json:"logFile"`
	PidFile  nxpkgpath.AbsoluteSystemPath `json:"pidFile"`
	SockFile nxpkgpath.AbsoluteSystemPath `json:"sockFile"`
}
...
// On windows, ':' is an invalid path character, but you can, and Nxpkgrepo does,
...
func (d *DaemonClient) GetChangedOutputs(ctx context.Context, hash string, repoRelativeOutputGlobs []string) ([]string, int, error) {
	...
	resp, err := d.client.GetChangedOutputs(ctx, &nxpkgdprotocol.GetChangedOutputsRequest{
		Hash:        hash,
...
func (d *DaemonClient) NotifyOutputsWritten(ctx context.Context, hash string, repoRelativeOutputGlobs hash.TaskOutputs, timeSaved int) error {
	...
	_, err := d.client.NotifyOutputsWritten(ctx, &nxpkgdprotocol.NotifyOutputsWrittenRequest{
		Hash:                 hash,
...
func (d *DaemonClient) Status(ctx context.Context) (*Status, error) {
	resp, err := d.client.Status(ctx, &nxpkgdprotocol.StatusRequest{})
	if err != nil {
		return nil, err
	}
