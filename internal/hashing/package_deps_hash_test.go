...
func getFixture(id int) nxpkgpath.AbsoluteSystemPath {
	cwd, _ := os.Getwd()
	root := nxpkgpath.AbsoluteSystemPath(filepath.VolumeName(cwd) + string(os.PathSeparator))
	checking := nxpkgpath.AbsoluteSystemPath(cwd)
	...
	for checking != root {
		...
		if !errors.Is(err, os.ErrNotExist) {
			...
			for _, file := range files {
				fileName := nxpkgpath.RelativeSystemPath(file.Name())
				if strings.Index(fileName.ToString(), fmt.Sprintf("%02d-", id)) == 0 {
					return nxpkgpath.AbsoluteSystemPath(fixtureDirectory.Join(fileName))
				}
			}
		}
...
func TestSpecialCharacters(t *testing.T) {
	...
	newlinePath := nxpkgpath.AnchoredUnixPath("new\nline").ToSystemPath()
	quotePath := nxpkgpath.AnchoredUnixPath("\"quote\"").ToSystemPath()
	...
	tests := []struct {
		name        string
		rootPath    nxpkgpath.AbsoluteSystemPath
		filesToHash []nxpkgpath.AnchoredSystemPath
		want        map[nxpkgpath.AnchoredUnixPath]string
		wantErr     bool
	}{
		{
			name:     "Quotes",
...
func getTraversePathInTests(rootPath nxpkgpath.AbsoluteSystemPath) (nxpkgpath.RelativeUnixPath, error) {
...
func Test_gitHashObject(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			filesToHash: []nxpkgpath.AnchoredSystemPath{
				nxpkgpath.AnchoredUnixPath("../root.json").ToSystemPath(),
				nxpkgpath.AnchoredUnixPath("child.json").ToSystemPath(),
				nxpkgpath.AnchoredUnixPath("grandchild/grandchild.json").ToSystemPath(),
			},
			want: map[nxpkgpath.AnchoredUnixPath]string{
		...
		},
		{
			...
			filesToHash: []nxpkgpath.AnchoredSystemPath{
				nxpkgpath.AnchoredUnixPath("null.json").ToSystemPath(),
			},
			want:    nil,
			wantErr: true,
		},
		{
			...
			rootPath: fixturePath,
			filesToHash: []nxpkgpath.AnchoredSystemPath{
				nxpkgpath.AnchoredUnixPath("nonexistent.json").ToSystemPath(),
			},
			want:    nil,
			wantErr: true,
		},
...
func requireGitCmd(t *testing.T, repoRoot nxpkgpath.AbsoluteSystemPath, args ...string) {
	t.Helper()
...
func TestGetPackageDeps(t *testing.T) {
	...
	type PackageDepsOptions struct {
		...
		PackagePath nxpkgpath.AnchoredSystemPath
	...
	tests := []struct {
		...
		expected map[nxpkgpath.AnchoredUnixPath]string
	}{
...
func Test_getPackageFileHashesFromProcessingGitIgnore(t *testing.T) {
	...
	root := t.TempDir()
	repoRoot := nxpkgpath.AbsoluteSystemPathFromUpstream(root)
	pkgName := nxpkgpath.AnchoredUnixPath("child-dir/libA").ToSystemPath()
	type fileHash struct {
		contents string
		hash     string
	}
	files := map[nxpkgpath.AnchoredUnixPath]fileHash{
	...
	files[nxpkgpath.AnchoredUnixPath("child-dir/libA/.gitignore")] = fileHash{contents: "", hash: "3237694bc3312ded18386964a855074af7b066af"}
...
func Test_manuallyHashFiles(t *testing.T) {
	...
	type args struct {
		rootPath     nxpkgpath.AbsoluteSystemPath
		files        []nxpkgpath.AnchoredSystemPath
		allowMissing bool
	}
	tests := []struct {
		name    string
		...
		// Tests for allowMissing = true
		{
			...
			args: args{
				rootPath:     testDir,
				files:        []nxpkgpath.AnchoredSystemPath{"non-existent-file.txt"},
				allowMissing: true,
			},
			want:    map[nxpkgpath.AnchoredUnixPath]string{},
			wantErr: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got map[nxpkgpath.AnchoredUnixPath]string
			var err error
