...
import (
	...
	"github.com/nxpkg/cli/internal/doublestar"
	"github.com/nxpkg/cli/internal/encoding/gitoutput"
	"github.com/nxpkg/cli/internal/fs"
	"github.com/nxpkg/cli/internal/globby"
...
// to `git hash-object`.
func gitHashObject(anchor nxpkgpath.AbsoluteSystemPath, filesToHash []nxpkgpath.AnchoredSystemPath) (map[nxpkgpath.AnchoredUnixPath]string, error) {
	fileCount := len(filesToHash)
	output := make(map[nxpkgpath.AnchoredUnixPath]string, fileCount)
	...
	if fileCount > 0 {
		...
		// The API of this method specifies that we return a `map[nxpkgpath.AnchoredUnixPath]string`.
...
func manuallyHashFiles(rootPath nxpkgpath.AbsoluteSystemPath, files []nxpkgpath.AnchoredSystemPath, allowMissing bool) (map[nxpkgpath.AnchoredUnixPath]string, error) {
	hashObject := make(map[nxpkgpath.AnchoredUnixPath]string, len(files))
...
func getPackageFileHashesFromGitIndex(rootPath nxpkgpath.AbsoluteSystemPath, packagePath nxpkgpath.AnchoredSystemPath) (map[nxpkgpath.AnchoredUnixPath]string, error) {
	var result map[nxpkgpath.AnchoredUnixPath]string
	...
	var filesToHash []nxpkgpath.AnchoredSystemPath
...
func gitStatus(rootPath nxpkgpath.AbsoluteSystemPath) (map[nxpkgpath.AnchoredUnixPath]statusCode, error) {
	cmd := exec.Command(
		"git",               // Using `git` from $PATH,
	...
	output := make(map[nxpkgpath.AnchoredUnixPath]statusCode, len(entries))
	convertedRootPath := nxpkgpath.AbsoluteSystemPathFromUpstream(rootPath.ToString())
	...
	for _, entry := range entries {
		...
		pathFromStatus := nxpkgpath.AnchoredUnixPathFromUpstream(statusEntry.GetField(gitoutput.Path))
		var outputPath nxpkgpath.AnchoredUnixPath
...
func safeCompileIgnoreFile(filepath nxpkgpath.AbsoluteSystemPath) (*gitignore.GitIgnore, error) {
...
func getPackageFileHashesFromProcessingGitIgnore(rootPath nxpkgpath.AbsoluteSystemPath, packagePath nxpkgpath.AnchoredSystemPath, inputs []string) (map[nxpkgpath.AnchoredUnixPath]string, error) {
	result := make(map[nxpkgpath.AnchoredUnixPath]string)
	absolutePackagePath := packagePath.RestoreAnchor(rootPath)
	...
	err = fs.Walk(absolutePackagePath.ToStringDuringMigration(), func(name string, isDir bool) error {
		convertedName := nxpkgpath.AbsoluteSystemPathFromUpstream(name)
...
func gitLsTree(rootPath nxpkgpath.AbsoluteSystemPath) (map[nxpkgpath.AnchoredUnixPath]string, error) {
	cmd := exec.Command(
		"git",     // Using `git` from $PATH,
	...
	output := make(map[nxpkgpath.AnchoredUnixPath]string, len(entries))

	for _, entry := range entries {
		lsTreeEntry := gitoutput.LsTreeEntry(entry)
		output[nxpkgpath.AnchoredUnixPathFromUpstream(lsTreeEntry.GetField(gitoutput.Path))] = lsTreeEntry[2]
	}
...
func getPackageFileHashesFromInputs(rootPath nxpkgpath.AbsoluteSystemPath, packagePath nxpkgpath.AnchoredSystemPath, inputs []string) (map[nxpkgpath.AnchoredUnixPath]string, error) {
	absolutePackagePath := packagePath.RestoreAnchor(rootPath)
	...
	// Add in package.json and nxpkg.json to input patterns. Both file paths are relative to pkgPath
	//
	// - package.json is an input because if the `scripts` in
	// 		the package.json change (i.e. the tasks that nxpkg executes), we want
	// 		a cache miss, since any existing cache could be invalid.
	// - nxpkg.json because it's the definition of the tasks themselves. The root nxpkg.json
	...
	calculatedInputs = append(calculatedInputs, "nxpkg.json")
	...
	filesToHash := make([]nxpkgpath.AnchoredSystemPath, len(absoluteFilesToHash))
	for i, rawPath := range absoluteFilesToHash {
		...
		filesToHash[i] = nxpkgpath.AnchoredSystemPathFromUpstream(relativePathString)
	}
...
func getTraversePath(rootPath nxpkgpath.AbsoluteSystemPath) (nxpkgpath.RelativeUnixPath, error) {
	...
	return nxpkgpath.RelativeUnixPathFromUpstream(trimmedTraversePath), nil
}

// Don't shell out if we already know where you are in the repository.
// `memoize` is a good candidate for generics.
func memoizeGetTraversePath() func(nxpkgpath.AbsoluteSystemPath) (nxpkgpath.RelativeUnixPath, error) {
	cacheMutex := &sync.RWMutex{}
	cachedResult := map[nxpkgpath.AbsoluteSystemPath]nxpkgpath.RelativeUnixPath{}
	cachedError := map[nxpkgpath.AbsoluteSystemPath]error{}

	return func(rootPath nxpkgpath.AbsoluteSystemPath) (nxpkgpath.RelativeUnixPath, error) {
		cacheMutex.RLock()
