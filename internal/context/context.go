...
func BuildPackageGraph(repoRoot nxpkgpath.AbsoluteSystemPath, rootPackageJSON *fs.PackageJSON, packageManagerName string) (*Context, error) {
	c := &Context{}
...
func (c *Context) parsePackageJSON(repoRoot nxpkgpath.AbsoluteSystemPath, pkgJSONPath nxpkgpath.AbsoluteSystemPath) error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	...
	if pkgJSONPath.FileExists() {
		...
		pkg.PackageJSONPath = nxpkgpath.AnchoredSystemPathFromUpstream(relativePkgJSONPath)
		pkg.Dir = nxpkgpath.AnchoredSystemPathFromUpstream(filepath.Dir(relativePkgJSONPath))
...
func (c *Context) externalWorkspaceDeps() map[nxpkgpath.AnchoredUnixPath]map[string]string {
	workspaces := make(map[nxpkgpath.AnchoredUnixPath]map[string]string, len(c.WorkspaceInfos.PackageJSONs))
	for _, pkg := range c.WorkspaceInfos.PackageJSONs {
...
func (c *Context) populateExternalDeps(repoRoot nxpkgpath.AbsoluteSystemPath, rootPackageJSON *fs.PackageJSON, warnings *Warnings) error {
