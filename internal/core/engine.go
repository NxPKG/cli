...
type Task struct {
	Name string
	// TaskDefinition contains the config for the task from nxpkg.json
...
type Engine struct {
	...
	// isSinglePackage is used to load nxpkg.json correctly
...
func (e *Engine) getTaskDefinition(pkg string, taskName string, taskID string) (*Task, error) {
	...
	if err != nil {
		if pkg != util.RootPkgName {
			// If there was no nxpkg.json in the workspace, fallback to the root nxpkg.json
	...
	// An error here means nxpkg.json exists, but didn't define the task.
...
func (e *Engine) Prepare(options *EngineBuildingOptions) error {
	...
	for len(traversalQueue) > 0 {
		...
		if pkg == util.RootPkgName && !e.rootEnabledTasks.Includes(taskName) {
			return fmt.Errorf("%v needs an entry in nxpkg.json before it can be depended on because it is a task run from the root package", taskID)
		}
		...
		if pkg != ROOT_NODE_NAME {
			if _, ok := e.completeGraph.WorkspaceInfos.PackageJSONs[pkg]; !ok {
				...
				// or the workspace really doesn't exist and nxpkg.json is misconfigured.
		...
		// https: //nxpkg.github.io/repo/docs/reference/command-line-reference/run#--only
		if tasksOnly {
...
func (e *Engine) ValidatePersistentDependencies(graph *graph.CompleteGraph, concurrency int) error {
	...
	if len(validationErrors) > 0 {
		...
		return fmt.Errorf("You have %v persistent tasks but `nxpkg` is configured for concurrency of %v. Set --concurrency to at least %v", persistentCount, concurrency, persistentCount+1)
	}
...
func (e *Engine) getTaskDefinitionChain(taskID string, taskName string) ([]fs.BookkeepingTaskDefinition, error) {
	...
	if e.isSinglePackage {
		if len(taskDefinitions) == 0 {
			return nil, fmt.Errorf("Could not find \"%s\" in root nxpkg.json", taskID)
		}
		return taskDefinitions, nil
	}

	// If the taskID is a root task (e.g. //#build), we don't need to look
	// for a workspace task, since these can only be defined in the root nxpkg.json.
	taskIDPackage, _ := util.GetPackageTaskFromId(taskID)
	if taskIDPackage != util.RootPkgName && taskIDPackage != ROOT_NODE_NAME {
		// If there is an error, we can ignore it, since nxpkg.json config is not required in the workspace.
		if workspaceNxpkgJSON, err := e.completeGraph.GetNxpkgConfigFromWorkspace(taskIDPackage, e.isSinglePackage); err != nil {
			// swallow the error where the config file doesn't exist, but bubble up other things
			if !errors.Is(err, os.ErrNotExist) {
				return nil, err
			}
		} else {
			// Run some validations on a workspace nxpkg.json. Note that these validations are on
			// the whole struct, and not relevant to the taskID we're looking at right now.
			validationErrors := workspaceNxpkgJSON.Validate([]fs.NxpkgJSONValidation{
				validateNoPackageTaskSyntax,
				validateExtends,
			})

			if len(validationErrors) > 0 {
				fullError := errors.New("Invalid nxpkg.json")
			...
			if workspaceDefinition, ok := workspaceNxpkgJSON.Pipeline[taskName]; ok {
	...
	if len(taskDefinitions) == 0 {
		return nil, fmt.Errorf("Could not find \"%s\" in root nxpkg.json or \"%s\" workspace", taskID, taskIDPackage)
	}

	return taskDefinitions, nil
}

func validateNoPackageTaskSyntax(nxpkgJSON *fs.NxpkgJSON) []error {
	errors := []error{}

	for taskIDOrName := range nxpkgJSON.Pipeline {
...
func validateExtends(nxpkgJSON *fs.NxpkgJSON) []error {
	extendErrors := []error{}
	extends := nxpkgJSON.Extends
