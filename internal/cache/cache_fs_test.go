...
import (
	...
	"github.com/nxpkg/cli/internal/cacheitem"
...
func TestPut(t *testing.T) {
	...
	src := nxpkgpath.AbsoluteSystemPath(t.TempDir())
	...
	files := []nxpkgpath.AnchoredSystemPath{
		nxpkgpath.AnchoredUnixPath("child/").ToSystemPath(),       // childDir
		nxpkgpath.AnchoredUnixPath("child/a").ToSystemPath(),      // aPath,
		nxpkgpath.AnchoredUnixPath("b").ToSystemPath(),            // bPath,
		nxpkgpath.AnchoredUnixPath("child/link").ToSystemPath(),   // srcLinkPath,
		nxpkgpath.AnchoredUnixPath("child/broken").ToSystemPath(), // srcBrokenLinkPath,
		nxpkgpath.AnchoredUnixPath("child/circle").ToSystemPath(), // circlePath
	}

	dst := nxpkgpath.AbsoluteSystemPath(t.TempDir())
	dr := &dummyRecorder{}
...
func assertFileMatches(t *testing.T, orig nxpkgpath.AbsoluteSystemPath, copy nxpkgpath.AbsoluteSystemPath) {
	t.Helper()
...
func TestFetch(t *testing.T) {
	...
	cacheDir := nxpkgpath.AbsoluteSystemPath(t.TempDir())
	hash := "the-hash"
	...
	srcBrokenLinkTarget := nxpkgpath.AnchoredUnixPath("missing").ToSystemPath()
	assert.NilError(t, srcBrokenLinkPath.Symlink(srcBrokenLinkTarget.ToString()), "Symlink")

	circlePath := childDir.Join("circle")
	srcCircleLinkTarget := nxpkgpath.AnchoredUnixPath("../child").ToSystemPath()
	...
	inputFiles := []nxpkgpath.AnchoredSystemPath{
		nxpkgpath.AnchoredUnixPath("some-package/child/").ToSystemPath(),       // childDir
		nxpkgpath.AnchoredUnixPath("some-package/child/a").ToSystemPath(),      // aPath,
		nxpkgpath.AnchoredUnixPath("some-package/b").ToSystemPath(),            // bPath,
		nxpkgpath.AnchoredUnixPath("some-package/child/link").ToSystemPath(),   // srcLinkPath,
		nxpkgpath.AnchoredUnixPath("some-package/child/broken").ToSystemPath(), // srcBrokenLinkPath,
		nxpkgpath.AnchoredUnixPath("some-package/child/circle").ToSystemPath(), // circlePath
	}

	putErr := cache.Put(cacheDir.UntypedJoin(hash), hash, 0, inputFiles)
	assert.NilError(t, putErr, "Put")

	outputDir := nxpkgpath.AbsoluteSystemPath(t.TempDir())
