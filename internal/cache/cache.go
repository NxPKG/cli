...
type Cache interface {
	...
	Fetch(anchor nxpkgpath.AbsoluteSystemPath, hash string, files []string) (ItemStatus, []nxpkgpath.AnchoredSystemPath, error)
	Exists(hash string) ItemStatus
	// Put caches files for a given hash
	Put(anchor nxpkgpath.AbsoluteSystemPath, hash string, duration int, files []nxpkgpath.AnchoredSystemPath) error
	Clean(anchor nxpkgpath.AbsoluteSystemPath)
	CleanAll()
	Shutdown()
}
...
func DefaultLocation(repoRoot nxpkgpath.AbsoluteSystemPath) nxpkgpath.AbsoluteSystemPath {
	return repoRoot.UntypedJoin("node_modules", ".cache", "nxpkg")
}
...
// resolveCacheDir calculates the location nxpkg should use to cache artifacts,
// based on the options supplied by the user.
func (o *Opts) resolveCacheDir(repoRoot nxpkgpath.AbsoluteSystemPath) nxpkgpath.AbsoluteSystemPath {
	if o.OverrideDir != "" {
...
func New(opts Opts, repoRoot nxpkgpath.AbsoluteSystemPath, client client, recorder analytics.Recorder, onCacheRemoved OnCacheRemoved) (Cache, error) {
...
func newSyncCache(opts Opts, repoRoot nxpkgpath.AbsoluteSystemPath, client client, recorder analytics.Recorder, onCacheRemoved OnCacheRemoved) (Cache, error) {
...
func (mplex *cacheMultiplexer) Put(anchor nxpkgpath.AbsoluteSystemPath, key string, duration int, files []nxpkgpath.AnchoredSystemPath) error {
...
func (mplex *cacheMultiplexer) storeUntil(anchor nxpkgpath.AbsoluteSystemPath, key string, duration int, files []nxpkgpath.AnchoredSystemPath, stopAt int) error {
...
func (mplex *cacheMultiplexer) Fetch(anchor nxpkgpath.AbsoluteSystemPath, key string, files []string) (ItemStatus, []nxpkgpath.AnchoredSystemPath, error) {
...
func (mplex *cacheMultiplexer) Clean(anchor nxpkgpath.AbsoluteSystemPath) {
	for _, cache := range mplex.caches {
		cache.Clean(anchor)
	}
}
