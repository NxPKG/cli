...
func (sr *errorResp) PutArtifact(hash string, body []byte, duration int, tag string) error {
	sr.t.Helper()
	outdir := nxpkgpath.AbsoluteSystemPathFromUpstream(sr.t.TempDir())
...
func TestRestoreTar(t *testing.T) {
	...
	expectedFiles := []nxpkgpath.AnchoredSystemPath{
		nxpkgpath.AnchoredUnixPath("extra-file").ToSystemPath(),
		nxpkgpath.AnchoredUnixPath("my-pkg").ToSystemPath(),
		nxpkgpath.AnchoredUnixPath("my-pkg/some-file").ToSystemPath(),
		nxpkgpath.AnchoredUnixPath("my-pkg/link-to-extra-file").ToSystemPath(),
		nxpkgpath.AnchoredUnixPath("my-pkg/broken-link").ToSystemPath(),
	}
...
func Test_httpCache_Put(t *testing.T) {
	...
	assert.ErrorIs(
		t,
		cache.Put(root, "000", 10, []nxpkgpath.AnchoredSystemPath{"one", "two"}),
		clientErr,
	...
	assert.ErrorIs(
		t,
		cache.Put(root, "000", 10, []nxpkgpath.AnchoredSystemPath{"one", "two", "missing"}),
		os.ErrNotExist,
	...
	assert.ErrorIs(
		t,
		cache.Put(root, "000", 10, []nxpkgpath.AnchoredSystemPath{"missing", "one", "two"}),
		os.ErrNotExist,
