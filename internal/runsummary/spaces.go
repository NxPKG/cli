...
func newSpacesClient(spaceID string, api spacesAPIClient) *spacesClient {
	...
	if !c.api.IsLinked() {
		c.errors = append(c.errors, fmt.Errorf("Error: experimentalSpaceId is enabled, but repo is not linked to API. Run `nxpkg link` or `nxpkg login` first"))
		return c
	}
...
type spacesRunPayload struct {
	...
	Type           string              `json:"type,omitempty"`           // hardcoded to "NXPKG"
	ExitCode       *int                `json:"exitCode,omitempty"`       // exit code for the full run
	Command        string              `json:"command,omitempty"`        // the thing that kicked off the nxpkg run
	...
	Client         spacesClientSummary `json:"client"`                   // Details about the nxpkg client
...
func newSpacesRunCreatePayload(rsm *Meta) *spacesRunPayload {
	...
	return &spacesRunPayload{
		...
		Client: spacesClientSummary{
			ID:      "nxpkg",
			Name:    "Nxpkg",
			Version: rsm.RunSummary.NxpkgVersion,
		},
	}
}
