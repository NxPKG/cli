...
import (
	...
	internalUI "github.com/nxpkg/cli/internal/ui"
...
func (rsm *Meta) printExecutionSummary(ui cli.Ui) {
	...
	if cached == attempted && attempted > 0 {
		...
		if terminalProgram == "Apple_Terminal" {
			fallbackNxpkgColor := color.New(color.FgHiMagenta, color.Bold).SprintFunc()
			maybeFullNxpkg = fallbackNxpkgColor(">>> FULL NXPKG")
		} else {
			maybeFullNxpkg = internalUI.Rainbow(">>> FULL NXPKG")
		}
	}
	...
	lineData := []summaryLine{
		...
		{header: "Time", trailer: util.Sprintf("%v${RESET} %v", duration, maybeFullNxpkg)},
	}
