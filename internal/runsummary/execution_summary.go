...
import (
	...
	"github.com/nxpkg/cli/internal/chrometracing"
...
// TaskExecutionSummary contains data about the state of a single task in a nxpkg run.
...
// executionSummary is the state of the entire `nxpkg run`. Individual task state in `Tasks` field
type executionSummary struct {
	...
	command   string                       // a synthesized nxpkg command to produce this invocation
	repoPath  nxpkgpath.RelativeSystemPath // the (possibly empty) path from the nxpkgrepo root to where the command was run
...
// newExecutionSummary creates a executionSummary instance to track events in a `nxpkg run`.`
func newExecutionSummary(command string, repoPath nxpkgpath.RelativeSystemPath, start time.Time, tracingProfile string) *executionSummary {
	if tracingProfile != "" {
...
// writeChromeTracing writes to a profile name if the `--profile` flag was passed to nxpkg run
func writeChrometracing(filename string, terminal cli.Ui) error {
	...
	name := fmt.Sprintf("nxpkg-%s.trace", time.Now().Format(time.RFC3339))
	if filename != "" {
		name = filename
	}
	...
	// so we have to account for that before converting to nxpkgpath.AbsoluteSystemPath
