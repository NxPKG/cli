// Package cmd holds the root cobra command for nxpkg
package cmd
...
import (
	...
	"github.com/nxpkg/cli/internal/run"
	"github.com/nxpkg/cli/internal/signals"
	"github.com/nxpkg/cli/internal/nxpkgstate"
	"github.com/nxpkg/cli/internal/util"
)

func initializeOutputFiles(helper *cmdutil.Helper, parsedArgs *nxpkgstate.ParsedArgsFromRust) error {
	if parsedArgs.Trace != "" {
...
// RunWithExecutionState runs nxpkg with the ParsedArgsFromRust that is passed from the Rust side.
func RunWithExecutionState(executionState *nxpkgstate.ExecutionState, nxpkgVersion string) int {
	util.InitPrintf()
	// TODO: replace this with a context
	signalWatcher := signals.NewWatcher()
	helper := cmdutil.NewHelper(nxpkgVersion, &executionState.CLIArgs)
