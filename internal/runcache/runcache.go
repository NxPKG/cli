...
type LogReplayer = func(logger hclog.Logger, output *cli.PrefixedUi, logFile nxpkgpath.AbsoluteSystemPath)
...
// RunCache represents the interface to the cache for a single `nxpkg run`
...
func New(cache cache.Cache, repoRoot nxpkgpath.AbsoluteSystemPath, opts Opts, colorCache *colorcache.ColorCache) *RunCache {
	rc := &RunCache{
...
type TaskCache struct {
	...
	LogFileName       nxpkgpath.AbsoluteSystemPath
}
...
func (tc *TaskCache) SaveOutputs(ctx context.Context, logger hclog.Logger, terminal cli.Ui, duration int) error {
	...
	relativePaths := make([]nxpkgpath.AnchoredSystemPath, len(filesToBeCached))
...
func defaultLogReplayer(logger hclog.Logger, output *cli.PrefixedUi, logFileName nxpkgpath.AbsoluteSystemPath) {
