...
import (
	...
	"github.com/nxpkg/cli/internal/spinner"
...
func RealRun(
	...
	defer func() {
		_ = spinner.WaitFor(ctx, nxpkgCache.Shutdown, base.UI, "...Finishing writing to cache...", 1500*time.Millisecond)
	}()
	colorCache := colorcache.New()

	runCache := runcache.New(nxpkgCache, base.RepoRoot, rs.Opts.runcacheOpts, colorCache)
...
func (ec *execContext) exec(ctx gocontext.Context, packageTask *nodes.PackageTask, ui cli.Ui, outWriter io.Writer) (*runsummary.TaskExecutionSummary, error) {
	...
	// TODO(gsoltis): if/when we fix https://github.com/vercel/nxpkg/issues/937
	...
	passThroughEnv.Add("NXPKG_HASH", hash)
