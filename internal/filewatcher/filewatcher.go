...
type Event struct {
	Path      nxpkgpath.AbsoluteSystemPath
	EventType FileEvent
}
...
type Backend interface {
	AddRoot(root nxpkgpath.AbsoluteSystemPath, excludePatterns ...string) error
	Events() <-chan Event
	Errors() <-chan error
	Close() error
	Start() error
...
func New(logger hclog.Logger, repoRoot nxpkgpath.AbsoluteSystemPath, backend Backend) *FileWatcher {
...
// events.
func (fw *FileWatcher) AddRoot(root nxpkgpath.AbsoluteSystemPath, excludePatterns ...string) error {
