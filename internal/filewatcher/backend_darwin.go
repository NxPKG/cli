...
func (f *fseventsBackend) AddRoot(someRoot nxpkgpath.AbsoluteSystemPath, excludePatterns ...string) error {
...
func waitForCookie(root nxpkgpath.AbsoluteSystemPath, events <-chan []fsevents.Event, timeout time.Duration) error {
	// This cookie needs to be in a location that we're watching, and at this point we can't guarantee
	// what the root is, or if something like "node_modules/.cache/nxpkg" would make sense. As a compromise, ensure
	// that we clean it up even in the event of a failure.
	cookiePath := root.UntypedJoin(".nxpkg-cookie")
...
func toFileEvent(flags fsevents.EventFlags, path nxpkgpath.AbsoluteSystemPath) FileEvent {
