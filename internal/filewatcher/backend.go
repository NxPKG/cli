...
// watch it.
func (f *fsNotifyBackend) onFileAdded(name nxpkgpath.AbsoluteSystemPath) error {
	info, err := name.Lstat()
	if err != nil {
...
func (f *fsNotifyBackend) watchRecursively(root nxpkgpath.AbsoluteSystemPath, excludePatterns []string, addMode watchAddMode) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fsNotifyBackend) AddRoot(root nxpkgpath.AbsoluteSystemPath, excludePatterns ...string) error {
