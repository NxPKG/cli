...
type CookieJar struct {
	...
	dir     nxpkgpath.AbsoluteSystemPath
	serial  uint64
	mu      sync.Mutex
	cookies map[nxpkgpath.AbsoluteSystemPath]chan error
	closed  bool
}
...
func NewCookieJar(cookieDir nxpkgpath.AbsoluteSystemPath, timeout time.Duration) (*CookieJar, error) {
	...
	return &CookieJar{
		timeout: timeout,
		dir:     cookieDir,
		cookies: make(map[nxpkgpath.AbsoluteSystemPath]chan error),
	}, nil
}
...
func (cj *CookieJar) OnFileWatchError(err error) {
	...
	cj.cookies = make(map[nxpkgpath.AbsoluteSystemPath]chan error)
}
...
func (cj *CookieJar) notifyCookie(cookie nxpkgpath.AbsoluteSystemPath, err error) {
	cj.mu.Lock()
...
func touchCookieFile(cookie nxpkgpath.AbsoluteSystemPath) error {
