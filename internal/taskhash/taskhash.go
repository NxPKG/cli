...
import (
	...
	"github.com/nxpkg/cli/internal/hashing"
	"github.com/nxpkg/cli/internal/inference"
	"github.com/nxpkg/cli/internal/nodes"
...
type Tracker struct {
	...
	PackageInputsExpandedHashes map[string]map[nxpkgpath.AnchoredUnixPath]string
	...
	packageTaskOutputs     map[string][]nxpkgpath.AnchoredSystemPath
...
func NewTracker(rootNode string, globalHash string, envAtExecutionStart env.EnvironmentVariableMap, pipeline fs.Pipeline) *Tracker {
	return &Tracker{
		...
		packageTaskOutputs:     make(map[string][]nxpkgpath.AnchoredSystemPath),
...
func (th *Tracker) CalculateFileHashes(
	...
	hashObjects := make(map[string]map[nxpkgpath.AnchoredUnixPath]string, len(hashTasks))
...
func (th *Tracker) CalculateTaskHash(logger hclog.Logger, packageTask *nodes.PackageTask, dependencySet dag.Set, frameworkInference bool, args []string) (string, error) {
	...
	if frameworkInference {
		...
		if framework != nil {
			...
			excludePrefix, exists := th.EnvAtExecutionStart["NXPKG_CI_VENDOR_ENV_KEY"]
...
func (th *Tracker) GetExpandedInputs(packageTask *nodes.PackageTask) map[nxpkgpath.AnchoredUnixPath]string {
	expandedInputs := th.PackageInputsExpandedHashes[packageTask.TaskID]
	inputsCopy := make(map[nxpkgpath.AnchoredUnixPath]string, len(expandedInputs))
...
func (th *Tracker) GetExpandedOutputs(taskID string) []nxpkgpath.AnchoredSystemPath {
	th.mu.RLock()
	defer th.mu.RUnlock()
	outputs, ok := th.packageTaskOutputs[taskID]

	if !ok {
		return []nxpkgpath.AnchoredSystemPath{}
	}
...
func (th *Tracker) SetExpandedOutputs(taskID string, outputs []nxpkgpath.AnchoredSystemPath) {
	th.mu.Lock()
	defer th.mu.Unlock()
