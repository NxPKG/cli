...
type Lockfile interface {
	...
	ResolvePackage(workspacePath nxpkgpath.AnchoredUnixPath, name string, version string) (Package, error)
...
type closureMsg struct {
	workspace nxpkgpath.AnchoredUnixPath
	closure   mapset.Set
}

// AllTransitiveClosures computes closures for all workspaces
func AllTransitiveClosures(
	workspaces map[nxpkgpath.AnchoredUnixPath]map[string]string,
	lockFile Lockfile,
) (map[nxpkgpath.AnchoredUnixPath]mapset.Set, error) {
	...
	closures := make(map[nxpkgpath.AnchoredUnixPath]mapset.Set, len(workspaces))
...
func transitiveClosure(
	workspaceDir nxpkgpath.AnchoredUnixPath,
...
func transitiveClosureHelper(
	wg *errgroup.Group,
	workspacePath nxpkgpath.AnchoredUnixPath,
	lockfile Lockfile,
...
func rustTransitiveDeps(content []byte, packageManager string, workspaces map[nxpkgpath.AnchoredUnixPath]map[string]string, resolutions map[string]string) (map[nxpkgpath.AnchoredUnixPath]mapset.Set, error) {
	...
	resolvedWorkspaces := make(map[nxpkgpath.AnchoredUnixPath]mapset.Set, len(workspaceDeps))
	for workspace, dependencies := range workspaceDeps {
		...
		workspacePath := nxpkgpath.AnchoredUnixPath(workspace)
