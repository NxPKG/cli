...
func CheckedToAbsoluteSystemPath(s string) (nxpkgpath.AbsoluteSystemPath, error) {
	if filepath.IsAbs(s) {
		return nxpkgpath.AbsoluteSystemPath(s), nil
	}
...
func ResolveUnknownPath(root nxpkgpath.AbsoluteSystemPath, unknown string) nxpkgpath.AbsoluteSystemPath {
	if filepath.IsAbs(unknown) {
		return nxpkgpath.AbsoluteSystemPath(unknown)
	}
	return root.UntypedJoin(unknown)
}

// UnsafeToAbsoluteSystemPath directly converts a string to an AbsoluteSystemPath
func UnsafeToAbsoluteSystemPath(s string) nxpkgpath.AbsoluteSystemPath {
	return nxpkgpath.AbsoluteSystemPath(s)
}

// UnsafeToAnchoredSystemPath directly converts a string to an AbsoluteSystemPath
func UnsafeToAnchoredSystemPath(s string) nxpkgpath.AnchoredSystemPath {
	return nxpkgpath.AnchoredSystemPath(s)
}
...
// into and out of the nxpkgpath.AbsoluteSystemPath type.
func AbsoluteSystemPathFromUpstream(s string) nxpkgpath.AbsoluteSystemPath {
	return nxpkgpath.AbsoluteSystemPath(s)
}

// GetCwd returns the calculated working directory after traversing symlinks.
func GetCwd(cwdRaw string) (nxpkgpath.AbsoluteSystemPath, error) {
	if cwdRaw == "" {
		var err error
		cwdRaw, err = os.Getwd()
		if err != nil {
...
func TempDir(subDir string) nxpkgpath.AbsoluteSystemPath {
	return nxpkgpath.AbsoluteSystemPath(os.TempDir()).UntypedJoin(subDir)
}

// GetUserConfigDir returns the platform-specific common location
// for configuration files that belong to a user.
func GetUserConfigDir() nxpkgpath.AbsoluteSystemPath {
	configHome := AbsoluteSystemPathFromUpstream(xdg.ConfigHome)
	return configHome.UntypedJoin("nxpkgrepo")
}
