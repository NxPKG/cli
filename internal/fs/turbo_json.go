...
type rawNxpkgJSON struct {
	...
	// Pipeline is a map of Nxpkg pipeline entries which define the task graph
...
// pristineNxpkgJSON is used when marshaling a NxpkgJSON object into a json string
...
type pristineNxpkgJSON struct {
	GlobalDependencies   []string                        `json:"globalDependencies,omitempty"`
	GlobalEnv            []string                        `json:"globalEnv,omitempty"`
	GlobalPassThroughEnv []string                        `json:"globalPassThroughEnv"`
	GlobalDotEnv         nxpkgpath.AnchoredUnixPathArray `json:"globalDotEnv"`
...
// NxpkgJSON represents a nxpkg.json configuration file
type NxpkgJSON struct {
	GlobalDeps           []string
	GlobalEnv            []string
...
type rawTaskWithDefaults struct {
	...
	DotEnv         nxpkgpath.AnchoredUnixPathArray `json:"dotEnv"`
}
...
type taskDefinitionHashable struct {
	...
	DotEnv                  nxpkgpath.AnchoredUnixPathArray
}
...
// LoadNxpkgConfig loads, or optionally, synthesizes a NxpkgJSON instance
func LoadNxpkgConfig(dir nxpkgpath.AbsoluteSystemPath, rootPackageJSON *PackageJSON, includeSynthesizedFromRootPackageJSON bool) (*NxpkgJSON, error) {
	// If the root package.json stil has a `nxpkg` key, log a warning and remove it.
	if rootPackageJSON.LegacyNxpkgConfig != nil {
		log.Printf("[WARNING] \"nxpkg\" in package.json is no longer supported. Migrate to %s by running \"npx @nxpkg/codemod create-nxpkg-config\"\n", configFile)
		rootPackageJSON.LegacyNxpkgConfig = nil
	}

	var nxpkgJSON *NxpkgJSON
	nxpkgFromFiles, err := readNxpkgConfig(dir.UntypedJoin(configFile))
	...
	if !includeSynthesizedFromRootPackageJSON && err != nil {
		...
		if errors.Is(err, os.ErrNotExist) {
			return nil, errors.Wrap(err, fmt.Sprintf("Could not find %s. Follow directions at https://nxpkg.github.io/repo/docs to create one", configFile))
		...
		return nxpkgFromFiles, nil
	} else if errors.Is(err, os.ErrNotExist) {
		// nxpkg.json doesn't exist, but we're going try to synthesize something
		nxpkgJSON = &NxpkgJSON{
			GlobalEnv: []string{},
		...
		pipeline := make(Pipeline)
		for taskID, taskDefinition := range nxpkgFromFiles.Pipeline {
		...
		nxpkgJSON = nxpkgFromFiles
		nxpkgJSON.Pipeline = pipeline
	}

	for scriptName := range rootPackageJSON.Scripts {
		if !nxpkgJSON.Pipeline.HasTask(scriptName) {
			...
			nxpkgJSON.Pipeline[taskName] = BookkeepingTaskDefinition{
	...
	return nxpkgJSON, nil
}

// NxpkgJSONValidation is the signature for a validation function passed to Validate()
type NxpkgJSONValidation func(*NxpkgJSON) []error

// Validate calls an array of validation functions on the NxpkgJSON struct.
// The validations can be customized by the caller.
func (tj *NxpkgJSON) Validate(validations []NxpkgJSONValidation) []error {
	allErrors := []error{}
...
// readNxpkgConfig reads nxpkg.json from a provided path
func readNxpkgConfig(nxpkgJSONPath nxpkgpath.AbsoluteSystemPath) (*NxpkgJSON, error) {
	// If the configFile exists, use that
	if nxpkgJSONPath.FileExists() {
		nxpkgJSON, err := readNxpkgJSON(nxpkgJSONPath)
		if err != nil {
	...
	// If there's no nxpkg.json, return an error.
	return nil, os.ErrNotExist
}

// readNxpkgJSON reads the configFile in to a struct
func readNxpkgJSON(path nxpkgpath.AbsoluteSystemPath) (*NxpkgJSON, error) {
	file, err := path.Open()
	if err != nil {
		return nil, err
	}
	...
	err = jsonc.Unmarshal(data, &nxpkgJSON)
...
// see whether a field was actually in the underlying nxpkg.json
...
func MergeTaskDefinitions(taskDefinitions []BookkeepingTaskDefinition) (*TaskDefinition, error) {
	...
	// Set the default, because the 0-value will be false, and if no nxpkg.jsons had
...
// nxpkg.json into a TaskDefinition struct
func (btd *BookkeepingTaskDefinition) UnmarshalJSON(data []byte) error {
	...
	for _, dependency := range task.DependsOn {
		if strings.HasPrefix(dependency, envPipelineDelimiter) {
			log.Printf("[DEPRECATED] Declaring an environment variable in \"dependsOn\" is deprecated, found %s. Use the \"env\" key or use `npx @nxpkg/codemod migrate-env-var-dependencies`.\n", dependency)
	...
	if task.DotEnv != nil {
		...
		btd.TaskDefinition.DotEnv = make(nxpkgpath.AnchoredUnixPathArray, 0, len(task.DotEnv))

		// Port the raw dotEnv values in.
		for _, dotEnvPath := range task.DotEnv {
			typeCheckedPath, err := nxpkgpath.CheckedToAnchoredUnixPath(dotEnvPath)
			if err != nil {
				return err
			}
...
// UnmarshalJSON deserializes the contents of nxpkg.json into a NxpkgJSON struct
func (tj *NxpkgJSON) UnmarshalJSON(data []byte) error {
	raw := &rawNxpkgJSON{}
	...
	for _, value := range raw.GlobalDependencies {
		if strings.HasPrefix(value, envPipelineDelimiter) {
			log.Printf("[DEPRECATED] Declaring an environment variable in \"globalDependencies\" is deprecated, found %s. Use the \"globalEnv\" key or use `npx @nxpkg/codemod migrate-env-var-dependencies`.\n", value)
	...
	// turn the set into an array and assign to the NxpkgJSON struct fields.
	...
	if raw.GlobalDotEnv != nil {
		tj.GlobalDotEnv = make(nxpkgpath.AnchoredUnixPathArray, 0, len(raw.GlobalDotEnv))
...
// UnmarshalJSON deserializes the remoteCache key from nxpkg.json into an in-memory struct
...
// MarshalJSON converts a NxpkgJSON into the equivalent json object in bytes
// note: we go via rawNxpkgJSON so that the output format is correct.
// This is used by `nxpkg prune` to generate a pruned nxpkg.json
// and also by --summarize & --dry=json to serialize the known config
// into something we can print to screen
func (tj *NxpkgJSON) MarshalJSON() ([]byte, error) {
	raw := pristineNxpkgJSON{}
