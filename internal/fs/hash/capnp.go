// Package hash contains the capnp schema and hashing functions for the nxpkg cache
//
...
//
// in crates/nxpkgrepo-lib/src/hash or run `make nxpkg-capnp` in the `cli` directory.
package hash
...
type TaskHashable struct {
	...
	PackageDir   nxpkgpath.AnchoredUnixPath
	Task         string
	Outputs      TaskOutputs
	PassThruArgs []string
...
type GlobalHashable struct {
	...
	GlobalFileHashMap    map[nxpkgpath.AnchoredUnixPath]string
...
func HashTaskHashable(task *TaskHashable) (string, error) {
	...
	taskMsg, err := nxpkg_capnp.NewRootTaskHashable(seg)
	if err != nil {
		return "", err
	}
	...
	{
		var envMode nxpkg_capnp.TaskHashable_EnvMode
		switch task.EnvMode {
		case util.Infer:
			panic("task inferred status should have already been resolved")
		case util.Loose:
			envMode = nxpkg_capnp.TaskHashable_EnvMode_loose
		case util.Strict:
			envMode = nxpkg_capnp.TaskHashable_EnvMode_strict
		}
...
func HashGlobalHashable(global *GlobalHashable) (string, error) {
	...
	globalMsg, err := nxpkg_capnp.NewRootGlobalHashable(seg)
	if err != nil {
		return "", err
	}
	...
	{
		var envMode nxpkg_capnp.GlobalHashable_EnvMode
		switch global.EnvMode {
		case util.Infer:
			envMode = nxpkg_capnp.GlobalHashable_EnvMode_infer
		case util.Loose:
			envMode = nxpkg_capnp.GlobalHashable_EnvMode_loose
		case util.Strict:
			envMode = nxpkg_capnp.GlobalHashable_EnvMode_strict
		}
...
func HashLockfilePackages(packages []lockfile.Package) (string, error) {
	...
	globalMsg, err := nxpkg_capnp.NewRootLockFilePackages(seg)
	if err != nil {
		return "", err
	}
...
func HashFileHashes(fileHashes map[nxpkgpath.AnchoredUnixPath]string) (string, error) {
	arena := capnp.SingleSegment(nil)
	...
	globalMsg, err := nxpkg_capnp.NewRootFileHashes(seg)
	if err != nil {
		return "", err
	}
...
func HashMessage(msg *capnp.Message) (string, error) {
	...
	// _ = nxpkgpath.AbsoluteSystemPath(".nxpkg/go-hash").WriteFile(bytes, 0644)
...
func assignSortedHashMap(packages map[nxpkgpath.AnchoredUnixPath]string, setEntry func(int, string, string) error) error {
	keys := make([]nxpkgpath.AnchoredUnixPath, len(packages))
...
func assignAnchoredUnixArray(paths nxpkgpath.AnchoredUnixPathArray, fn func(capnp.TextList) error, seg *capnp.Segment) error {
